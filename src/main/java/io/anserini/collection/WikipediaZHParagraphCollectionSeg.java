 ...
 *
 */
public class WikipediaZHParagraphCollectionSeg extends DocumentCollection
    implements SegmentProvider<WikipediaZHParagraphCollectionSeg.Document> {
  private static final Logger LOG = LogManager.getLogger(WikipediaZHParagraphCollectionSeg.class);
  ...
  public class FileSegment extends BaseFileSegment<Document> {
    ...
    public boolean hasNext() {
      ...
      while(!iterParagraph.hasNext()) {
        ...
        String paragraph = iterParagraph.next().trim(); // Trim and add the punctuation back in since we split on it.
        paragraph = paragraph.replaceAll("\\n+", " ");
        bufferedRecord = new WikipediaZHParagraphCollectionSeg.Document(node.get("id").asText() + "_" + String.valueOf(iterParagraph.nextIndex()), paragraph);
        while(!iterParagraph.hasNext()) {
			if (iterator.hasNext()) { // if bufferedReader contains JSON line objects, we parse the next JSON into node
			  node = iterator.next();
			  String text = node.get("text").asText();  
