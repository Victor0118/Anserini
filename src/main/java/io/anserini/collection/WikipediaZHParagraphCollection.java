 ...
 *
 */
public class WikipediaZHParagraphCollection extends DocumentCollection
    implements SegmentProvider<WikipediaZHParagraphCollection.Document> {
  private static final Logger LOG = LogManager.getLogger(WikipediaZHParagraphCollection.class);
  ...
  public class FileSegment extends BaseFileSegment<Document> {
    ...
    public boolean hasNext() {
      ...
      while(!iterParagraph.hasNext()) {
        ...
        bufferedRecord = new WikipediaZHParagraphCollection.Document(node.get("id").asText() + "_" + String.valueOf(iterParagraph.nextIndex()), paragraph);
        while(!iterParagraph.hasNext()) {
			if (iterator.hasNext()) { // if bufferedReader contains JSON line objects, we parse the next JSON into node
			  node = iterator.next();
			  String text = node.get("text").asText();  
